...
4. Codex may present a diff to a reviewer, and the reviewer approves or rejects strictly following project documentation guidelines

## Workflow Execution Strategy (MANDATORY)
When performing tasks, Codex **MUST**:
1. Analyze the task to identify independent subtasks
2. Select appropriate specialized agents from `.Codex/agents/` using:
...
5. Employ Git-based checkpoints: `git checkout -b Codex-session-[timestamp]-[purpose]` for version control
6. **Critical:** Ensure agent outputs are trackable with unique IDs when issues are identified

## Available Workflows
Check `.Codex/workflows/` for YAML workflow definitions. Choose workflows appropriate to the task complexity:
...
All agents are defined in `.Codex/agents/`. Key agents include:
...
## Report Format
All agent outputs must follow the template in `.Codex/template/report.md`.
